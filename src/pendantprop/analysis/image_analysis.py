...
class PendantDropAnalysis:
    ...
    def select_image(self):
        ...
        self.file_path = askopenfilename(filetypes=[("Image Files", "*.jpg;*.jpeg;*.png")])
    ...
    def analyse(self):
        ...
        self.de = w  #! important for calculation st
        ...
        self._draw_double_arrow_line(
            ...
            thickness=10,
            tip_length=0.05,
            text=f"de={self.de:.0f}px",  # Add the text for the line
        )
        ...
        cropped_image = cropped_image[
            int(top_left[1]) : int(bottom_left[1] - (self.de)),
        ...
        S = ds / self.de
        self.Hin = self._calculate_Hin(S)
        de_scaled = self.de * self.scale  # mm -> pixels
        surface_tension = (
            self.density * self.gravity_constant * (de_scaled**2) * self.Hin
        )
    ...
    def img2wo(self, img, vol_droplet):
        self.raw_image = img
        self.process_image()
        st = self.analyse()
        if st > 20:
            if self.check_diameter():
                ...
                print(
                    f"dia: {self.needle_diameter_px_measured} px, too big probably droplet sticking.".ljust(
                        30
                    ),
                    end="\r",
                )
